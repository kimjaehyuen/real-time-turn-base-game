...
9. **키보드로 조작할 수 있다.**
   `1`~`4`로 아군 캐릭터를 선택하고(카드 우측 상단의 번호와 대응), `Q`는 일반공격, `E`는 스킬,
   `Space`는 필살기를 사용합니다. 대상이 필요하면 마우스로 대상 카드를 클릭해서 지정합니다
   (`src/main.ts`의 keydown 핸들러).
...
- 하단 액션 패널: 턴이 활성화된 아군 전원의 일반공격/스킬 선택 (대상이 필요하면 카드를 클릭해서 지정)
- 상단 컨트롤: 일시정지, 배속(x1/x2/x4), 전투 재시작
- 키보드: `1`~`4` 캐릭터 선택 · `Q` 일반공격 · `E` 스킬 · `Space` 필살기
