...
7. **대상을 선택하는 동안에는 모든 캐릭터의 행동 게이지가 멈춘다.**
   스킬(혹은 일반공격/필살기)의 대상을 고르는 중에는 `BattleEngine.setTargetSelectionPaused(true)`가 호출되어
   아군/적 가릴 것 없이 게이지 충전과 적 AI의 행동 진행이 전부 멈춥니다 — 대상을 고르는 동안 전황이
   바뀌지 않습니다. (단, 필살기는 이 정지와도 무관하게 계속 즉시 사용 가능하고, 화상/중독 같은
   시간 기반 상태이상은 규칙 5에 따라 이때도 계속 흐릅니다.)

8. **아군은 최대 4명, 적은 최대 5마리까지 동시에 편성될 수 있다.**
   이 데모는 그 한도를 꽉 채워 아군 4명(전사/마도사/힐러/암살자) vs 적 5마리로 시작합니다
   (`src/engine/characters.ts`의 `createInitialRoster()`).

9. **필살기는 키보드로도 사용할 수 있다.**
   캐릭터/대상 선택은 항상 마우스로 하고, `1`~`4`(카드 우측 상단의 번호와 대응)는 그 슬롯의
   필살기가 준비되어 있을 때만 즉시 사용하는 단축키입니다 (`src/main.ts`의 keydown 핸들러).
   준비되면 카드의 번호 배지가 보라색으로 강조됩니다.

10. **피해량은 화면 좌/우상단에 표시되고, 그 편이 다시 행동할 때마다 초기화된다.**
    전투 로그 대신, 아군이 적에게 입힌 피해는 우상단에, 적이 아군에게 입힌 피해는 좌상단에
    표시됩니다. 한쪽이 새 행동(일반공격/스킬/필살기)을 시작하면 그 편의 표시만 0으로 리셋된 뒤
    이번 행동의 피해량으로 다시 채워집니다 — 즉 누적 총합이 아니라 "가장 최근 행동의 피해량"입니다
    (`BattleEngine.lastPlayerDamage` / `lastEnemyDamage`).
...
src/
  engine/
    ...
    characters.ts      캐릭터 9종(아군 4 + 적 5)의 일반공격/스킬/필살기 정의
    battle.ts          게이지/대기열/필살기 인터럽트/AI를 포함한 전투 시뮬레이션 엔진
  ui/
    types.ts           UI 전용 타입 (대상 선택 상태 등)
    render.ts           DOM 렌더링 (구조 변경과 수치 갱신을 분리해 매 프레임 버튼이 재생성되지 않도록 함, 좌/우상단 피해량 표시 포함)
...
- 캐릭터/대상 선택은 전부 마우스 클릭: 캐릭터 카드 아래 필살기 버튼(에너지가 가득 차면 언제든 사용 가능),
  하단 액션 패널의 일반공격/스킬 버튼, 대상 카드 클릭
- 키보드: 필살기가 준비된 캐릭터가 있으면 `1`~`4`로 즉시 사용 (카드 번호와 대응)
- 상단 컨트롤: 일시정지, 배속(x1/x2/x4), 전투 재시작
- 상단 SP 바: 아군이 공유하는 스킬 포인트 (최대 10)
- 화면 좌상단: 적이 아군에게 입힌 최근 피해량 / 우상단: 아군이 적에게 입힌 최근 피해량
